...
function onBegin()
    disableMovement()
    say("What a nice view...")
    celeste.SetFlag("youth_valley-1-intro_camera-1", true)
    wait(1.0)
    celeste.SetFlag("youth_valley-1-intro_camera-2", true)
    wait(1.0)
    celeste.SetFlag("youth_valley-1-intro_camera-2", true)
    wait(1.0)
    celeste.SetFlag("camera_reset", true)
    enableMovement()
end
...
-- Helper function to lerp the camera over time
function lerpCamera(targetX, targetY, duration)
...
-- Your main cutscene execution
function onBegin()
    if celeste.getFlag("youth_valley_1_camera_focus") then
        -- Call the lerp function (Target X, Target Y, Duration in seconds)
        lerpCamera(720, 288, 1.5)
    end
end